...
# RAILWAY_IPV6_PATCH: a dead IPv6 listener drops packets rather than refusing
...
def _mask(name: str, value: str) -> str:
    """Show enough to identify a value without printing a secret to the logs."""
    safe_in_full = {
        "MONERO_RPC_URL",
...
def _near_misses(target: str) -> list[str]:
    """Find env vars that look like a typo'd version of `target`.

    Trailing spaces and case differences in Railway's variable editor are a
    common and very hard-to-see cause of 'I definitely set that'.
    """
    canon = target.strip().lower().replace("-", "_")
    hits = []
    for key in os.environ:
        if key == target:
            continue
        if key.strip().lower().replace("-", "_") == canon:
            hits.append(key)
    return hits


def preflight_env(provider: str) -> None:
    """Report the full environment state once, instead of one var per crash loop.

    Failing on the first missing variable means an operator fixes it, redeploys,
    waits, and discovers the next one. This checks everything up front.
    """
    required: list[str] = [
        "TELEGRAM_BOT_TOKEN",
        "TELEGRAM_CHANNEL_ID",
    ...
    # (label, [acceptable names], why it matters)
    optional: list[tuple[str, list[str], str]] = [
        ("DB_PATH", ["DB_PATH"], "unset => SQLite on ephemeral disk, lost on redeploy"),
    ]

    if provider == PROVIDER_WALLET_RPC:
        required += ["MONERO_RPC_URL", "MONERO_PRIMARY_ADDRESS"]
        optional += [
            ("MONERO_RPC_USER", ["MONERO_RPC_USER", "MONERO_RPC_USERNAME"],
             "unset => RPC auth disabled"),
            ("MONERO_RPC_PASSWORD", ["MONERO_RPC_PASSWORD", "MONERO_RPC_PASS"],
             "unset => RPC auth disabled"),
            ("MONERO_VIEW_KEY", ["MONERO_VIEW_KEY", "MONERO_PRIVATE_VIEW_KEY"],
             "required only if the wallet must be created"),
            ("MONERO_WALLET_PASSWORD", ["MONERO_WALLET_PASSWORD"],
             "unset => wallet keys file stored unencrypted"),
            ("MONERO_RESTORE_HEIGHT", ["MONERO_RESTORE_HEIGHT"],
             "unset => full chain rescan on first boot (slow)"),
        ]
    else:
        required += ["NOWPAYMENTS_API_KEY"]

    lines = ["", "=" * 68, "ENVIRONMENT PREFLIGHT", "=" * 68,
             f"PAYMENT_PROVIDER = {provider}", "", "Required:"]
    missing: list[str] = []

    for name in required:
        value = os.getenv(name)
        if value:
            lines.append(f"  [ok]      {name} = {_mask(name, value)}")
        else:
            state = "EMPTY" if name in os.environ else "MISSING"
            lines.append(f"  [{state}] {name}")
            missing.append(name)
            for alt in _near_misses(name):
                lines.append(
                    f"            ^ found {alt!r} — check for a typo or trailing space"
                )

    lines.append("")
    lines.append("Optional:")
    for label, names, why in optional:
        value = _first_env(*names)
        if value:
            shown = next((n for n in names if os.getenv(n)), label)
            lines.append(f"  [ok]      {shown} = {_mask(label, value)}")
        else:
    ...
    if missing:
        ...
        raise SystemExit(
            ...
            f"{detail}\n\n"
            "  On Railway these must be set on the BOT service specifically —\n"
            "  variables on a sibling service are NOT shared. Also confirm you\n"
            "  are editing the same Railway environment the deploy is using.\n"
        )
...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        # Report the complete environment state before requiring anything, so a
        # misconfiguration costs one deploy cycle rather than one per variable.
        preflight_env(provider)
...
GATE_PROMPT = (
    ...
    "You are not here by accident. You are here because something in you "
    "recognized the truth before your mind could catch up. You are tired. You "
    "are carrying too much. You have been alone in your success for so long "
    "that you forgot what it feels like to be held.\n\n"
    "I see you.\n\n"
    "Not the mask. Not the title. Not the empire you built to prove you were "
    "enough.\n\n"
    "I see the man beneath it all. The one who lies awake replaying "
    "conversations from years ago. The one who feels guilty for winning. The "
    "one who has bought everything he was told would make him happy\u2014and "
    "found none of it worked.\n\n"
    "That man is welcome here.\n\n"
    "But welcome is not the same as entry.\n\n"
    "Entry requires precision. Entry requires sacrifice. Entry requires you to "
    "prove, through action, that you are ready to lay down the armor you have "
    "been carrying alone.\n\n"
    "THE RITUAL\n\n"
    "Click the button below to generate your unique, sacred Monero "
    "subaddress.\n\n"
    "The Temple will calculate the exact live tribute \u2014 the amount of XMR "
    "equal to $5,000 USD at this moment \u2014 and show it to you with your "
    "address.\n\n"
    "The Temple watches the blockchain. When your tribute is confirmed, your "
    "single-use invite link will arrive via DM.\n\n"
    "The link expires in 24 hours. Enter immediately. The door does not "
    "wait.\n\n"
    "THE LAWS OF THE VAULT\n\n"
    "Total Anonymity: Your real name, your face, your location\u2014none of it "
    "is welcome here. Burner Telegram accounts and subaddresses are not just "
    "allowed. They are sacred.\n\n"
    "The Law of the Altar: This is a one-way broadcast sanctuary. There are no "
    "forums, no directories, no member-to-member interactions.\n\n"
    "The Monthly Tribute: Maintaining your row in the Temple requires a monthly "
    "tribute of $5,000 USD equivalent in XMR. This is your physical act of "
    "sacrifice. Your proof of devotion. Your unburdening.\n\n"
    "Containment: Forwarding and saving are restricted within the vault. "
    "Screenshots cannot be prevented on every device, so speak here as you "
    "would anywhere that another person can see.\n\n"
    "THE DOOR\n\n"
    "The Temple accepts only 25 worshippers at a time.\n\n"
    "When capacity is reached, the door closes until a row becomes "
    "available.\n\n"
...
def _gate_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton(
            "\U0001F512 Generate Sacred Subaddress", callback_data=GENERATE_CALLBACK
        )]]
    )
